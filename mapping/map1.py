...
for lt, ln, el in zip(lat, lon, elev, name):
    iframe = folium.IFrame(html=html_link % (name, name, el), width=200, height=100)
    fgv.add_child(folium.CircleMarker(location=[lt, ln], radius=6, popup=folium.Popup(iframe) + (str(el)+" m"),
...
map.save("Map_html_link_popup.html")